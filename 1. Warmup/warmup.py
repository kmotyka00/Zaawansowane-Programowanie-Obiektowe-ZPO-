...
def f_indexing_and_slicing(data: List[int]) -> Tuple[int, int, List[int]]:
    return data[0], data[-1], data[0:len(data) - 1]
...
def f_str_formatting(array: List[float]) -> str:
    st = ""
    for elem in array:
        if elem is not array[-1]:
            st = st + "{:.2f}{}".format(elem, " - ")
        else:
            st = st + "{:.2f}".format(elem)
    return st
...
def f_comprehensions(dct: Dict[int, int]) -> Tuple[int, Set[int]]:
    return sum(x for x in range(3, 10) if x != 5 or x != 7), {key for key in dct.keys() if 1 <= key <= 5}
